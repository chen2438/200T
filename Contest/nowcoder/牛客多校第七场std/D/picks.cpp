...
ll gettri(ll A,ll B,ll g){
	if (A==0 || B==0) return 0;
	return ((A*B-A-B+g)/2)%mod;
}
...
ll solve(ll n,ll m){
	...
	for (auto e:v){
		...
		ll g1=__gcd(e.r,e.i);
		if (m%(n/g1)) continue;
	//	cerr << (long long)A << " " << (long long)B << endl;
		ll g2=m/(n/g1);
		ll C=A/g1*g2,D=B/g1*g2;
		ans=(ans+2*gettri(A,B,g1)+2*gettri(C,D,g2)+(A-D)*(C-B)%mod+mod)%mod;
