...
const int base=998244353,mod=1000000009;
char s[N];
...
{
	return ((hsh[l]-hsh[r+1]*bin[r-l+1])%mod+mod)%mod;
}
...
void init()
{
	...
	repd(i,n,1) hsh[i]=(hsh[i+1]*base+c[i])%mod;
}
...
bool cmp(int x,int y)
{
	...
	rep(i,1,p)
	{
		LL tx=dx,ty=dy;
		int t=pos[i];
		if(nxt[x][a[x+t-1]]==x+t-1) tx=(dx-c[x+t-1]*bin[t-1]%mod+mod)%mod;
		if(nxt[y][a[y+t-1]]==y+t-1) ty=(dy-c[y+t-1]*bin[t-1]%mod+mod)%mod;
		LL X=(gethsh(x,x+t-1)+tx)%mod;
		LL Y=(gethsh(y,y+t-1)+ty)%mod;
		if(X!=Y)
		{
			l=pos[i-1],r=t-1;
			break;
	...
	while(l<=r)
	{
		LL X=(gethsh(x,x+mid-1)+dx)%mod;
		LL Y=(gethsh(y,y+mid-1)+dy)%mod;
		if(X==Y) l=mid+1;
		else r=mid-1;
	}
	--l;
