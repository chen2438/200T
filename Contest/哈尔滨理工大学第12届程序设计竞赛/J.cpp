...
inline BigInt mo(BigInt a,BigInt mod){
    BigInt c=a/mod;
    return a-c*mod;
}

BigInt quick_pow(BigInt a,BigInt b,BigInt mod){//a^b%mod
    BigInt ans=1;
    BigInt zero=1;zero=zero-1;
    //cout<<"zero="<<zero<<endl;
    BigInt base=a;
    while(b>zero){
        if(mo(b,2)==1) ans*=base,ans=mo(ans,mod);
        base*=base,base=mo(base,mod);
        b=b/2;
    }
    return ans;
}
